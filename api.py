...
def naiveinit(expr,dim_specs):
    ...
    indent = ""
    prod = 1
    lines.append(f"from math import *")
    for spec in dim_specs:
        q = spec.split(",")[-1]
        prod *= int(q)
    lines.append(f"result = [0.0] * {prod}\ni=0")
    for spec in dim_specs:
    ...
    lines.append(f"{indent}result[i] = {expr}")

    return "\n".join(lines)

def benchmark(expr, cycle):
    ncode = naiveinit(expr, cycle)
