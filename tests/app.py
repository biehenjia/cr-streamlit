...
streamlit.markdown("Input a univariate expression to benchmark evaluation time against alternatives.")
...
if streamlit.button("Benchmark Expression"):
    ...
    for i in range(7):
        ...
        result, time_taken = py.evalcr(benchexpr, p)
        compiled_time = py.benchmark(benchexpr, p)
        data.append({
            "Input Size": 10**i,
...
if streamlit.button("Evaluate Expression"):
    result, time_taken = py.evalcr(colexpr, colparam.split())
    streamlit.write(f"Last Value: {result[-1]}, Time taken: {time_taken:.2f} ms")
    code, time_taken = py.crgen(colexpr, colparam.split())
    streamlit.code(code)